...
class RuntimeUIAnalysisExportTests(unittest.TestCase):
    ...
    def test_learning_prompt_scaffolds_are_present(self):
        ...
        self.assertIn("核验状态", JS)
        self.assertIn("证据规则", JS)
        self.assertIn("7 天计划", JS)
    ...
    def test_compare_prompt_is_learning_focused(self):
        ...
        self.assertIn("核心思路、实现取舍、可借鉴点、不该照搬、融合路线", JS)
        self.assertIn("每个重要判断都要标注证据", JS)
    ...
    def test_single_repo_prompt_output_uses_learning_constraints(self):
        ...
        self.assertIn("核验状态", prompt)
        self.assertIn("证据规则", prompt)
        self.assertIn("README：已核验 / 未核验 / 未发现", prompt)
        self.assertIn("【已核验】", prompt)
        self.assertIn("【基于代码结构推测】", prompt)
        self.assertIn("【当前未核验】", prompt)
        self.assertIn("【当前信息不足】", prompt)
        ...
        self.assertIn("如果我写了“请给 7 天计划”", prompt)
        self.assertIn("如果我没有明确要求 7 天计划，不要默认输出", prompt)
    ...
    def test_batch_prompt_output_is_learning_first(self):
        ...
        self.assertIn("当前未核验", prompt)
        self.assertIn("每个重要判断都要标注证据", prompt)
        self.assertIn("先给总览表", prompt)
        self.assertIn("最值得学的点", prompt)
        self.assertIn("最值得借鉴的点", prompt)
        self.assertIn("最值得深读的前 3 个项目", prompt)
        self.assertIn("暂缓项目", prompt)
